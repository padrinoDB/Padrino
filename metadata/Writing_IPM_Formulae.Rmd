...
and has a few quirks in it due to the nature of working with strings in R/Perl.
...
For hierarchical models, this table stores the different levels of the hierarchical
...
entered as `size_yr` or `size_pl`. If a model contains multiple hierarchical effects,
...
generally discrete, may or may not have a set order, and ordered effects may or may
not be easily represented by a sequence of integers. Below we cover both cases.
...
"level_C", "Level_...")`. Note that each level is now encapsulated with quotation
marks, all levels are separated by commas, and everything is wrapped in `c()`. 
This is very important (though perhaps a bit lazy of me and the least "language-agnostic"
aspect of the database).

The above is a bit vague and confusing, so see the [Worked Examples](#examples) in the next section 
for a more concrete demonstration of how this works.

### Why all this confusion for hierarchical models?

The general goal of the framework we implemented for hierarchical models is to 
...
consequently lowering the risk of typos. A separate PDF explains how this 
...
hierarchical models can be found 
