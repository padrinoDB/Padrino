...
test_cases <- pdb$TestTargets[!is.na(pdb$TestTargets$precision), ]
...
# Count number of lambdas generated by each model so that we can know 
# roughly the number of unique kernels that can be generated from each
# PDB version

for(i in seq_along(good_db$Metadata$ipm_id)) {
  
  use_id <- good_db$Metadata$ipm_id[i]
  skip_t <- isTRUE(good_db$Metadata$.test_passed[i])
  ...
  if(!is.null(test_ipm[[1]]$msg$errors) ||
    ...
    if(!is.null(test_ipm[[1]]$msg$errors)) next
    
  } 
  
  if(!is.null(test_ipm[[1]]$result)) {
  ...
  if(use_id %in% test_cases$ipm_id && 
     !is.na(test_cases$precision[test_cases$ipm_id == use_id])) {
    ...
    test_rng    <- c(target_lam - test_tol, target_lam + test_tol)
    
    test_lam <- round(test_lam, digits = target_prec)
    
    # Roughly test for accuracy. in general, this should filter out anything
    # off by > 0.03
    res <- test_lam %between% test_rng
